...
void stateMachine()
{
    ...
    case State::STANDBY:
        handleStandbyState();
        break;
    case State::TEMP_HIGH:
        handleTempHighState();
        break;
    case State::MAIN_SCREEN:
        enterMainScreen();
        break;
    case State::MAIN_SCREEN_CNT:
        handleMainScreenCnt();
        break;
    case State::MENU_SCREEN:
        handleMenuScreen(str);
        break;
    case State::SUB_MENU_1:
        handleSubMenu1(str);
        break;
    case State::SUB_MENU_2:
        handleSubMenu2(str);
        break;
    case State::SYSTEM_SCREEN:
        enterSystemScreen();
        break;
    case State::SYSTEM_MENU:
        handleSystemMenu(str);
        break;
    case State::REBOOT_MENU:
        handleRebootMenu(str);
        break;
    case State::MAXWELD_SCREEN:
        handleMaxWeldScreen(str);
        break;
    case State::INVERT_SCREEN:
        handleInvertScreen();
        break;
    default:
        break; // Defensive — unknown state does nothing, WDT keeps ticking.
    }
}
...
void handleMainScreenCnt()
{
    ...
    if (digitalRead(PIN_AUTO_PULSE) && pData.pFlags.en_autoPulse) {
        if (!autoPulseTriggered) {
            ...
            sendWeldPulse(PIN_AUTO_PULSE, static_cast<milliseconds_t>(pData.autoPulseDelay) * 100,
                          WP_RETRIGGER_DELAY_MS, PULSE_ACTIVE_HIGH);
            displayMainScreen();
        }
    } else if (!digitalRead(PIN_FOOT_SWITCH)) {
        sendWeldPulse(PIN_FOOT_SWITCH, FS_TRIGGER_DELAY_MS, WP_RETRIGGER_DELAY_MS,
                      PULSE_ACTIVE_LOW);
    } else {
        autoPulseTriggered = false;
    }
...
void handleEncoderEvent(Event ev)
{
    if (ev == Event::ENCUP) {
        pData.pulseTime =
                (pData.pulseTime < pData.maxPulseTime) ? pData.pulseTime + 1 : pData.maxPulseTime;
    } else {
...
void handleMenuScreen(char* str)
{
    ...
    if (ev == Event::BTNDN) {
        ...
        if (menuState.selectedMainMenu == 0) {
            displayMenuType1(FPSTR(LS_APULSE), FPSTR(LS_MANAUTO), FPSTR(LS_DELAY),
                             FPSTR(LS_WELDSOUND), menuState.selectedMenu);
        } else if (menuState.selectedMainMenu == 1) {
            displayMenuType1(FPSTR(LS_BATTALM), FPSTR(LS_LOWALRM), FPSTR(LS_HIGHALRM),
                             FPSTR(LS_EXIT), 0);
        ...
        atomicSetEvent(Event::NONE);
        displayMenuType1(nullptr, FPSTR(LS_APULSE), FPSTR(LS_BATTALM1), FPSTR(LS_SHORTPLS1),
...
void handleSubMenu1(char* str)
{
    ...
    if (ev == Event::BTNDN) {
        if (menuState.selectedMainMenu == 0) {
            ...
            if (menuState.selectedSubMenu == 0) {
                ...
                displayMenuType2(FPSTR(LS_WELDSOUNDM), nullptr,
                                 pData.pFlags.en_autoPulse ? FPSTR(LS_SOUNDON)
                                                           : FPSTR(LS_SOUNDOFF));
        ...
        if (menuState.selectedMainMenu == 0) {
            if (ev == Event::ENCDN) {
                menuState.selectedMenu =
                        (menuState.selectedMenu == 0) ? 3 : menuState.selectedMenu - 1;
            } else {
                menuState.selectedMenu =
                        (menuState.selectedMenu == 3) ? 0 : menuState.selectedMenu + 1;
            }
            ...
            if (menuState.selectedMenu < 3) {
                ...
                displayMenuType1(FPSTR(LS_APULSE), FPSTR(LS_DELAY), FPSTR(LS_WELDSOUND),
                                 FPSTR(LS_NOMVOLT), 2);
            }
            ...
            if (ev == Event::ENCDN) {
                menuState.selectedMenu =
                        (menuState.selectedMenu == 0) ? 2 : menuState.selectedMenu - 1;
            } else {
                menuState.selectedMenu =
                        (menuState.selectedMenu == 2) ? 0 : menuState.selectedMenu + 1;
            }
            displayMenuType1(FPSTR(LS_BATTALM), FPSTR(LS_LOWALRM), FPSTR(LS_HIGHALRM),
                             FPSTR(LS_EXIT), menuState.selectedMenu);

        } else if (menuState.selectedMainMenu == 2) {
            if (ev == Event::ENCDN) {
                pData.shortPulseTime = (pData.shortPulseTime > MIN_SPULSE_TIME)
                                               ? pData.shortPulseTime - 1
                                               : MIN_SPULSE_TIME;
            } else {
...
void handleSubMenu2(char* str)
{
    ...
    if (ev == Event::BTNDN) {
        ...
        if (menuState.selectedMainMenu == 0) {
            if (menuState.selectedSubMenu == 0) {
                ...
                if (ev == Event::ENCDN) {
                    pData.nominalVoltage = (pData.nominalVoltage > MIN_NOMINAL_VOLTAGE_EE)
                                                   ? pData.nominalVoltage - 1
                                                   : MIN_NOMINAL_VOLTAGE_EE;
                } else {
                    pData.nominalVoltage = (pData.nominalVoltage < MAX_NOMINAL_VOLTAGE_EE)
                                                   ? pData.nominalVoltage + 1
                                                   : MAX_NOMINAL_VOLTAGE_EE;
                }
                displayMenuType2(FPSTR(LS_NOMVOLTMENU),
                                 valStr(str, pData.nominalVoltage, ValueFormat::BATTERY_ALARM),
                                 FPSTR(LS_VOLTAGE));
            ...
            if (menuState.selectedSubMenu == 0) {
                uint16_t maxLowAlarm = (pData.batteryhighAlarm > 0) ? pData.batteryhighAlarm - 1
                                                                    : MIN_BATT_ALARM_MV / 100;
                if (ev == Event::ENCDN) {
                    pData.batteryAlarm = (pData.batteryAlarm > MIN_BATT_ALARM_MV / 100)
                                                 ? pData.batteryAlarm - 1
                                                 : MIN_BATT_ALARM_MV / 100;
                } else {
                    pData.batteryAlarm = (pData.batteryAlarm < maxLowAlarm) ? pData.batteryAlarm + 1
                                                                            : maxLowAlarm;
                }
                displayMenuType2(FPSTR(LS_LOWALRMMENU),
                                 valStr(str, pData.batteryAlarm, ValueFormat::BATTERY_ALARM),
...
void enterSystemScreen()
{
    ...
    sysMenu = true;
    menuState.selectedMenu = 0;
    displayMenuType1(FPSTR(LS_SYSMENU), FPSTR(LS_MAXPULSE), FPSTR(LS_DISPLAY), FPSTR(LS_BOOT), 0);
}
...
void handleSystemMenu(char* str)
{
    ...
    if (ev == Event::BTNDN) {
        ...
        if (menuState.selectedMenu == 0) {
            ...
            menuState.selectedMenu = 0;
            displayMenuType1(FPSTR(LS_BOOTMENU), FPSTR(LS_REBOOT), FPSTR(LS_SAFERST),
                             FPSTR(LS_FULLRST), 0);
        }
        ...
        atomicSetEvent(Event::NONE);
        displayMenuType1(FPSTR(LS_SYSMENU), FPSTR(LS_MAXPULSE), FPSTR(LS_DISPLAY), FPSTR(LS_BOOT),
                         menuState.selectedMenu);
    }
}
...
void handleRebootMenu(char* /*str*/)
{
    ...
    if (ev == Event::BTNDN) {
        ...
        const __FlashStringHelper* actionMsg = (menuState.selectedMenu == 1)   ? FPSTR(LS_REBOOTSR)
        ...
        displayMenuType1(FPSTR(LS_BOOTMENU), FPSTR(LS_REBOOT), FPSTR(LS_SAFERST), FPSTR(LS_FULLRST),
                         menuState.selectedMenu);
    }
    atomicSetEvent(Event::NONE);
}
...
void handleMaxWeldScreen(char* str)
{
    ...
    if (ev == Event::BTNDN) {
        ...
        if (ev == Event::ENCDN) {
            pData.maxPulseTime =
                    (pData.maxPulseTime > MIN_PULSE_TIME) ? pData.maxPulseTime - 1 : MIN_PULSE_TIME;
        } else {
            pData.maxPulseTime = (pData.maxPulseTime < ABS_MAX_PULSE_TIME) ? pData.maxPulseTime + 1
                                                                           : ABS_MAX_PULSE_TIME;
        }
        ...
        displayMenuType2(FPSTR(LS_MAXPLSMENU),
                         valStr(str, pData.maxPulseTime, ValueFormat::PULSE_DELAY), FPSTR(LS_MS));
    }
    atomicSetEvent(Event::NONE);
}
...
void handleInvertScreen()
{
    ...
    if (ev == Event::BTNDN) {
        ...
        display.setRotation(pData.pFlags.en_oledInvert ? OLED_INVERT_ROTATION
                                                       : OLED_NORMAL_ROTATION);
        displayMenuType2(FPSTR(LS_INVERTMENU), nullptr,
                         pData.pFlags.en_oledInvert ? FPSTR(LS_SCRINV) : FPSTR(LS_SCRNORM));
    }
    atomicSetEvent(Event::NONE);
