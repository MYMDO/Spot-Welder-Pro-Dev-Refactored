...
typedef struct progData {
    ...
    uint16_t maxPulseTime;          // Maximum allowed pulse (ms) — user-set, clamped to [MIN_PULSE_TIME, ABS_MAX_PULSE_TIME]
...
static_assert(static_cast<uint8_t>(State::_COUNT)  == 13, "State enum changed — audit switch coverage");
static_assert(static_cast<uint8_t>(Event::_COUNT)  == 11, "Event enum changed — audit switch coverage");
...
#define DEF_NOMINAL_VOLTAGE_EE     80     // 8.0 V default nominalVoltage as stored in EEPROM (units of 100 mV)
...
void setTextProp(uint8_t size, int16_t x, int16_t y, uint16_t color = WHITE, bool invert = false);
...
void  sendWeldPulse(uint8_t sensePin, milliseconds_t delayEngage,
                    milliseconds_t delayRelease, bool senseActiveLevel = PULSE_ACTIVE_HIGH);
