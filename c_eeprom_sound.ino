...
char *valStr(char *str, uint16_t val, ValueFormat fType) {
    switch (fType) {
        case ValueFormat::BATTERY_ALARM:
            sprintf_P(str, PSTR("%u.%u"),   val / 10, val % 10);          break;  // 7.5 (decivolts)
        case ValueFormat::TEMP_ALARM:
            sprintf_P(str, PSTR("%4u"),     val);                          break;  // "  65"
        case ValueFormat::BATTERY_VOLTS:
            sprintf_P(str, PSTR("%2u.%02u"), val / 100, val % 100);        break;  // "12.45"
        case ValueFormat::BATTERY_AMPS:
            sprintf_P(str, PSTR("%5u"),     val);                          break;  // "  500"
        case ValueFormat::WELD_COUNT:
            sprintf_P(str, PSTR("%5u"),     val);                          break;  // "   42"
        case ValueFormat::TEMPERATURE:
            sprintf_P(str, PSTR("%5u"),     val);                          break;  // "   25"
        case ValueFormat::PULSE_DELAY:
            sprintf_P(str, PSTR("%3u"),     val);                          break;  // "  5"  (ms)
        case ValueFormat::SHORT_PULSE:
            sprintf_P(str, PSTR("%3u"),     val);                          break;  // "  0"  (%)
        case ValueFormat::AUTO_DELAY:
            sprintf_P(str, PSTR("%u.%u"),   val / 10, val % 10);          break;  // "2.0"  (seconds)
        default:
            str[0] = '?'; str[1] = '\0';                                  break;  // Defensive
    }
    return str;
}
...
#ifdef _DEVELOPMENT_
        Serial.print(F("EEPROM mismatch (stored=0x")); Serial.print(storedChecksum, HEX);
        Serial.print(F(" calc=0x"));                    Serial.print(calculatedChecksum, HEX);
...
void updateEEPROM() {
    ...
    if (calculatedChecksum == storedChecksum) {
        ...
        eepromDirty = false;
#ifdef _DEVELOPMENT_
        Serial.print(F("EEPROM OK (crc=0x")); Serial.print(calculatedChecksum, HEX); Serial.println(')');
#endif
        return;
    }
...
#ifdef _DEVELOPMENT_
    Serial.print(F("EEPROM written: 0x")); Serial.print(storedChecksum, HEX);
    Serial.print(F(" → 0x"));               Serial.println(calculatedChecksum, HEX);
#endif
}
...
void playHighVoltageAlarmSound() {
    playBeep(2000, 150); delay(50);
    playBeep(2500, 150); delay(50);
    playBeep(3000, 200);
}

void playLowVoltageAlarmSound() {
    playBeep(1500, 150); delay(50);
    playBeep(1000, 150); delay(50);
    playBeep(500,  200);
}
